...
Module({
  ...
  if (link !== null && getID.test(link[0])) {
    ...
    var song = await getSong(query[1]);
     ...
     .save('./song.mp3')
     .on('end', async () => {
      var song = await addInfo('./song.mp3',title,BOT_INFO.split(";")[0],"YD KING metadata",await skbuffer(thumbnail))
...
Module({
  ...
  const buttons = [{
      ...
      var {
          info,
          thumbnail
      } = await getJson("https://ydkingv1-network.vercel.app/api/youtube/details?video_id=" + message.list.split(";")[1]);
  ...
  if (message.list && message.list.startsWith("song") && message.list.includes(message.client.user.id.split("@")[0].split(":")[0])) {
         ...
         .save('./song.mp3')
         .on('end', async () => {
          var song = await addInfo('./song.mp3',title,BOT_INFO.split(";")[0],"YD KING audio downloader",await skbuffer(thumbnail))
          return await message.client.sendMessage(message.jid, {
              audio:song,
              mimetype: 'audio/mp4'
          }, {
